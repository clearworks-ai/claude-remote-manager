...
```bash
cat > ~/code/knowledge-sync/cc/sessions/nonprofit-dev-handoff-$(date +%Y-%m-%d-%H%M).md << 'HANDOFF'
---
type: handoff
agent: nonprofit-dev
created: <timestamp>
...
**On Session Start**, read latest handoff: `ls -t ~/code/knowledge-sync/cc/sessions/nonprofit-dev-handoff-*.md 2>/dev/null | head -1`
